...
class Actions():
    ...
    def bulk(model, data, cart, second_model):
        ...
        for _ in data:
            bulk_list.append(
                model(
                    product=second_model.objects.get(id=_['product']),
                    quantity=_['quantity'],
                    amount=_['amount'],
                    cart=cart
                )
...
class Crawler():
    ...
    def search_products(query, model, spy, serializer):
        ...
        result = model.objects.filter(spy(name__icontains=query) | spy(
            first_description__icontains=query) | spy(second_description__icontains=query))
        return result
