...
def create_get(req):
    ...
    if req.method == "GET":
        ...
        for i in data["results"]:
            i["product_img"] = f'https://backend.hairsenseretail.com/{i["product_img"]}'
...
def get_update_delete(req, index):
    ...
    if req.method == 'GET':
        data, status = Actions.get_single_product(
            serializer=CreateProductSerializer, model=Product, index=index, comments=Comment)
    ...
    if req.method == 'PUT':
        data, status = Actions.update(
            serializer=CreateProductSerializer, model=Product, index=index, data=req.data)
