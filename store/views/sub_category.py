...
def get_products(req, index):
    ...
    query = Product.objects.filter(
        sub_category=sub_category.id).order_by('-date_added')
...
def get_update_delete(req, index):
    ...
    if req.method == 'GET':
        data, status = Actions.get_single(
            serializer=GetCategorySubSerializer, model=SubCategory, index=index)
    ...
    if req.method == 'PUT':
        data, status = Actions.update(
            serializer=PostSubCategorySerializer, model=SubCategory, index=index, data=req.data)
